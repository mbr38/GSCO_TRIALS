...
5. **[RESOLVED — 23 May 2026, no fix required] `composite.overall_screening` rank-order:** Brasilia (0.213) currently ranks above Sapezal (0.179) on the headline screening score. This is the *opposite* of the intended demo narrative ("high-priority Amazon vs low-priority capital region"). Separate from A1 — likely a sub-aggregate weight or calibration issue downstream of the pollution/habitat scores themselves. Worth a deliberate "demo calibration" pass before any external demo.
    ...
    Investigation (Sapezal rank-order diagnostic, 23 May 2026) confirmed engine output is methodologically correct. Sapezal's 5 km buffer captures the town centre + stable post-conversion soy-belt landscape (47% existing cropland, 32% remnant trees, zero current natural→non-natural transitions in 2021-2026 window). Brasília's 43 km buffer legitimately overlaps Brasília National Park (KBA overlap 11.28%) and shows real urban NO₂ + VIIRS signals. The rank order reflects true screening-worthiness of the two specific AOIs and time window. The "high-priority Amazon" label on Sapezal describes the regional context, not what's in the buffer. If a future demo iteration needs a coordinate that matches the high-priority narrative, candidate frontier sites are noted in the investigation report (Novo Progresso, BR-163 corridor; Trairão/Itaituba area; Apuí region) — defer until needed.
