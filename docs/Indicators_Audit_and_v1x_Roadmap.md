...
## M-DOCS-CLEANUP-A1 methodology-sync register (31 May 2026)

Recorded here per M-DOCS-CLEANUP-A1. Four methodology milestones shipped across 30–31 May 2026; this register syncs their decisions into the audit doc in the same tabular pattern as the M-TREND-A1 register above. Rows are ordered chronologically by ship date. Each row records the decision, the previous → new state, the scope/impact, and an evidence link. These are additive sync entries — the per-indicator prose audit cards (§2–§4) and the GHG/Nature pillar-aggregate sections (§3.4, §4.8) are absorbed into the canonical spec docs as those milestones land; this register is the consolidated cross-reference.

| Milestone (ship date) | Decision | Previous → New state | Scope / impact | Evidence |
|---|---|---|---|---|
| **M-CH4-A1** (30 May 2026) | Reclassify CH₄ atmospheric anomaly-z from a scored GHG signal to **reference data**. | Scored input to the GHG composite via `CH₄_Context_Adjusted` → reference data, no severity scoring, surfaced in C5 alongside Hansen + ODIAC. | CH₄ anomaly-z fired at only 1/25 stratified sites at 5 km AOI and 0/25 at 15 km — not a reliable supplier-attributable screening signal at sub-pixel scale. `Core_GHG_Audit_Support` no longer carries a CH₄ term. Joins Hansen + ODIAC as the third `data_type="reference_dataset"` member of the GHG pillar. | `docs/M-CH4-A1_closed_entry.md`; `docs/ghg_odiac_validation.md` §8 (option c) + §10; `docs/Indicators_Computation_v4.md` §2.3 |
| **M-ATTRIB-A2** (31 May 2026) | Attributability **framing** pass for AOD (and the Air pillar generally) — prose/framing only, no scoring change. | AOD severity presented in user-facing prose as if it measured absolute air quality → reframed as a **supplier-attributable aerosol anomaly against regional context**. | No engine, weight, or seed change. Aligns AOD (and all z-score Air indicators) with the canonical severity-framing statement. The AOD validation §5.3 was reframed ("orthogonal" → "tracks local spatial contrast — by design") and a new §5.4 added. | `docs/aod_pm25_validation.md` §5.3 reframe + §5.4; `docs/M-ATTRIB-A2_closed_entry.md`; `docs/Indicators_Computation_v4.md` §0.7 (canonical framing statement) |
| **M-DIAG-A4 v2.0** (31 May 2026) | Replace the per-day z denominator (spatial std of the time-averaged ring field) with a **trailing climatology baseline σ** over `max(90, screening_window_length)` days, for the 8 Air indicators. | Spatial std of the time-averaged field (wrong scale for normalising per-day temporal deviations) → temporal σ from the trailing climatology period (correct scale). | 8 Air indicators (NO₂, SO₂, CO, HCHO, O₃, AAI, AOD, CAMS PM). A numerical-correctness fix — the spatial denominator collapsed ~10× too small at clean controls, inflating per-day z. **VIIRS is excluded** from this swap (`CLIMATOLOGY_DENOMINATOR_EXCLUDED_INDICATORS`) and is handled in Row 4. | `docs/M-DIAG-A4_closed_entry.md`; `docs/M-DIAG-A3_diagnosis_report.md` (with addendum) |
| **M-GHG-REDESIGN-A1** (31 May 2026) | Redesign VIIRS from the anomaly/z-score grammar to **persistence-weighted ring-relative sustained contrast**. | z-score detector with a spatial-std denominator (post-M-DIAG-A4 exclusion — a presence detector forced into an anomaly framework, with σ collapsing whether spatial or temporal) → `VIIRS_score = contrast_over_lit_window · persistence_factor(persistence)`, with a score-band severity (≥0.66 High, ≥0.33 Concern) rather than a z-score. | GHG composite reweighted to **0.60·Activity_Score (VIIRS) + 0.40·Combustion_Proxy** (`Core_GHG_Audit_Support_v1`). Intentional cross-pillar grammar divergence: VIIRS measures a sustained activity *stock* relative to surroundings, not a transient anomaly — not harmonised back onto the z-score path. | `docs/Indicators_Computation_v4.md` §2.2a + §2.3 |

*Document version 1.7 — 31 May 2026: appended the M-DOCS-CLEANUP-A1 methodology-sync register (four sync rows — M-CH4-A1, M-ATTRIB-A2, M-DIAG-A4 v2.0, M-GHG-REDESIGN-A1 — mirroring the M-TREND-A1 register pattern). Documentation-only; no engine, seed, or test change.*
